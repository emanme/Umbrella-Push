...
{
    public class CallBacksViewModel : ObservableBase
    {

        private List<Lead> _leads;

        public List<Lead> CallBacks
        {
            get { return _leads; }
            set
            {
        ...
        async Task methodRunPeriodically(string leadscategory)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("leads " + item.Name + " " + item.LastName + " " + leads.Count);
            }             leads = leads.Where(l => l.Category == leadscategory).ToList();             this.SetProperty(ref this._leads, leads); 
        }
